...
class BancoDeDados:
    def __init__(self, host="localhost", user="root", password="root", database="sudoku_db"):
        ...
        try:
            ...
            if self.conn.is_connected():
                ...
                raise mysql.connector.Error("Conexão falhou.")
    ...
    def inserir_ranking(self, nome, tempo):
        if self.cursor is not None:
            query = "INSERT INTO ranking (nome, tempo) VALUES (%s, %s)"
            self.cursor.execute(query, (nome, tempo))
            self.conn.commit()
        else:
            logging.warning("Conexão indisponível.")

    def listar_ranking(self):
        if self.cursor is not None:
            self.cursor.execute("SELECT nome, tempo FROM ranking ORDER BY tempo ASC LIMIT 5")
...
class SudokuGame:
    ...
    def choose_difficulty_screen(self):
        ...
        while not done:
            ...
            self.screen.blit(self.small_font.render("Fácil", True, (0, 0, 0)), (button_easy.x + 25, button_easy.y + 15))
            self.screen.blit(self.small_font.render("Médio", True, (0, 0, 0)), (button_medium.x + 20, button_medium.y + 15))
            self.screen.blit(self.small_font.render("Difícil", True, (0, 0, 0)), (button_hard.x + 15, button_hard.y + 15))
    ...
    def draw_grid(self):
        for i in range(10):
            ...
            color = (0, 0, 0) if i % 3 == 0 else (180, 180, 180)
    ...
    def show_ranking(self):
        ranking = self.db.listar_ranking()
        y = 560
        for idx, (nome, tempo) in enumerate(ranking, 1):
            text = self.small_font.render(f"{idx}. {nome} - {tempo}s", True, (0, 0, 128))
    ...
    def show_phase_info(self):
        ...
        self.screen.blit(msg, (270 - msg.get_width() // 2, 300))
        pygame.display.flip()
    ...
    def game_over_screen(self):
        while True:
            ...
            msg = self.small_font.render("Erros demais! Você perdeu.", True, (255, 255, 255))
            info = self.small_font.render("Pressione ESC para sair.", True, (255, 255, 255))

            self.screen.blit(title, (180, 250))
            self.screen.blit(msg, (160, 310))
            self.screen.blit(info, (160, 360))
            pygame.display.flip()
    ...
    def next_phase(self):
        if self.phase < self.max_phases:
            ...
            self.db.inserir_ranking(self.nome, tempo_total)

            while True:
                self.screen.fill((255, 255, 255))
                title = self.font.render("🏆 Parabéns!", True, (0, 128, 0))
                msg = self.small_font.render(f"{self.nome}, tempo total: {tempo_total}s", True, (0, 0, 0))
    ...
    def run_game(self):
        ...
        while True:
            ...
            tempo_jogo = int(time.time() - self.start_time)
            tempo_text = self.small_font.render(
                f"Tempo: {tempo_jogo}s | Fase {self.phase}",
                True, (50, 100, 50)
            )
            ...
            if self.board.selected:
                ...
                error_text = self.small_font.render(
                    f"Erros nesta célula: {erros} / {self.error_limit}",
                    True, (180, 30, 30)
                )
